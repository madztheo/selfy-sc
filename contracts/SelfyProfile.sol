    ...
    */
    function evolve(uint256 badgeId) public onlyRole(MINTER_ROLE)  {
        ...
        if (badgeIdToTraits[badgeId] == 0) {
            head[tokenId] = (head[tokenId] + block.number + block.timestamp) % MAX_HEAD;
        } else if (badgeIdToTraits[badgeId] == 1) {
            body[tokenId] = (head[tokenId] + block.number + block.timestamp) % MAX_BODY;
        } else if (badgeIdToTraits[badgeId] == 2) {
            accessory[tokenId] = (head[tokenId] + block.number + block.timestamp) %  MAX_ACCESSORY;
        } else if (badgeIdToTraits[badgeId] == 3) {
            glasses[tokenId] = (head[tokenId] + block.number + block.timestamp) %  MAX_GLASSES;
        } else if (badgeIdToTraits[badgeId] > 3) {
